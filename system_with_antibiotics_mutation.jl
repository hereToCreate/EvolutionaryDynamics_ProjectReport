...
chloramphenicol (C). Additionally, we incorporate plasmid transfer of the
ampicillin-resistance plasmid and the chloramphenicol-resistance plasmid,
creating double-resistant mutant E. coli with population density e12, intrinsic
growth rate β12 < min{β1, β2} and corresponding stimulated immune response x12.
...
e12 and the immune cells of x1, x2 and x12 and model its dynamics by

d/dt e1  = e1 r1(C) (1-γ/r1(C) (e1+e2+e12)) - p x1 e1 - μ e1 e2 - μ e1 e12
d/dt e2  = e2 r2(A) (1-γ/r2(A) (e1+e2+e12)) - p x2 e2 - μ e1 e2 - μ e2 e12
d/dt e12 = e12 (β12 - δ) (1-γ/(β12-δ) (e1+e2+e12) - p x12 e12 + 2 μ e1 e2 + μ e1 e12 + μ e2 e12
d/dt x1  = c e1 - b x1
d/dt x2  = c e2 - b x2
d/dt x12 = c e12 - b x12
...
μ = 0.0001 # per hour [3 * 10^(-5), 7 * 10^(-4)]
...
x2₀ = 0.
x12₀ = 0.
A₀  = 10  # μg/mL     0 -- 50 # 0, 2, 16, 64, (120,) 256
C₀  = 100 # μg/mL     0 -- 25 # 0, 2, 10, 50, 250, 1250, 6250, 31250, 156250

# solution arrays
e1_array = zeros(Float64, n_timepoints)
e2_array = zeros(Float64, n_timepoints)
e12_array = zeros(Float64, n_timepoints)
x1_array = zeros(Float64, n_timepoints)
x2_array = zeros(Float64, n_timepoints)
x12_array = zeros(Float64, n_timepoints)
...
function rhs(e1 :: Float64, e2 :: Float64, e12 :: Float64, x1 :: Float64,
	         x2 :: Float64, x12 :: Float64, A :: Float64, C :: Float64, t :: Float64)
	...
	ė2 = 0.
	if t >= tlag
		# note that the a plasmid could be transferred from e12 to e1 creating e1 and e12 etc.
		ė1 = e1 * r1 * (1 - γ/r1 * (e1+e2+e12)) - p * x1 * e1 - μ * e1 * e2 - μ * e1 * e12 # plasmid from e1 to e2
		ė2 = e2 * r2 * (1 - γ/r2 * (e1+e2+e12)) - p * x2 * e2 - μ * e1 * e2 - μ * e2 * e12 # plasmid from e2 to e1
	end

	return [ė1, ė2,
	        e12 * β12_minus_δ * (1 - γ/β12_minus_δ * (e1+e2+e12)) - p * x12 * e12 + 2 * μ * e1 * e2 + μ * e1 * e12 + μ * e2 * e12, # TODO reasonable?
			c * e1 - b * x1,
			c * e2 - b * x2,
			c * e12 - b * x12,
			-Vmax * A/(Km+A) * e1₀,
...
for d in 1:n_days  # day loop
	...
	for i in 1:n_timesteps_ti   # day hour loop
		...
		ω1 = rhs(e1_array[j], e2_array[j], e12_array[j], x1_array[j], x2_array[j],
		         x12_array[j], A_array[j], C_array[j], ti)
		ω2 = rhs(e1_array[i] + 0.5 * Δt * ω1[1], e2_array[j] + 0.5 * Δt * ω1[2],
		         e12_array[j] + 0.5 * Δt * ω1[3], x1_array[j] + 0.5 * Δt * ω1[4],
				 x2_array[j] + 0.5 * Δt * ω1[5], x12_array[j] + 0.5 * Δt * ω1[6],
				 A_array[j] + 0.5 * Δt * ω1[7], C_array[j] + 0.5 * Δt * ω1[8],
				 ti + Δt/2)
		ω3 = rhs(e1_array[j] + 0.5 * Δt * ω2[1], e2_array[j] + 0.5 * Δt * ω2[2],
		         e12_array[j] + 0.5 * Δt * ω2[3], x1_array[j] + 0.5 * Δt * ω2[4],
				 x2_array[j] + 0.5 * Δt * ω2[5], x12_array[j] + 0.5 * Δt * ω2[6],
				 A_array[j] + 0.5 * Δt * ω2[7], C_array[j] + 0.5 * Δt * ω2[8],
				 ti + Δt/2)
		ω4 = rhs(e1_array[j] + Δt * ω3[1], e2_array[j] + Δt * ω3[2],
		         e12_array[j] + Δt * ω3[3],
				 x1_array[j] + Δt * ω3[4], x2_array[j] + Δt * ω3[5],
				 x12_array[j] + Δt * ω3[6], A_array[j] + Δt * ω3[7],
				 C_array[j] + Δt * ω3[8], ti + Δt)
        ...
        e12_array[j+1] = e12_array[j] + Δt/6 * (ω1 + 2 * ω2 + 2 * ω3 + ω4)[3]
		x1_array[j+1] = x1_array[j] + Δt/6 * (ω1 + 2 * ω2 + 2 * ω3 + ω4)[4]
		x2_array[j+1] = x2_array[j] + Δt/6 * (ω1 + 2 * ω2 + 2 * ω3 + ω4)[5]
		x12_array[j+1] = x12_array[j] + Δt/6 * (ω1 + 2 * ω2 + 2 * ω3 + ω4)[6]
		A_array[j+1] = A_array[j] + Δt/6 * (ω1 + 2 * ω2 + 2 * ω3 + ω4)[7]
		C_array[j+1] = C_array[j] + Δt/6 * (ω1 + 2 * ω2 + 2 * ω3 + ω4)[8]
		...
		if i == n_timesteps_ti
			...
			e12_array[j+1] = e12_array[j+1]/100
			x1_array[j+1] = x1_array[j+1]/100
			x2_array[j+1] = x2_array[j+1]/100
			x12_array[j+1] = x12_array[j+1]/100
			e1₀ = e1_array[j+1]
		end
		ti += Δt
	end
...
println("")
println("average of e1 during last 2 days: ", sum(e1_array[100 * 28 * 24:n_timepoints])/length(sum(e1_array[100 * 28 * 24:n_timepoints])))
println("average of e2 during last 2 days: ", sum(e2_array[100 * 28 * 24:n_timepoints])/length(sum(e2_array[100 * 28 * 24:n_timepoints])))
println("average of e12 during last 2 days: ", sum(e12_array[100 * 28 * 24:n_timepoints])/length(sum(e12_array[100 * 28 * 24:n_timepoints])))
...
pic = PyPlot.plot(t_array./24, e12_array, color="red", label="e₁₂ (double-resistant E. coli)")
...
#PyPlot.plot(t_array./24, x12_array, color="orange", label="x₁₂ (immune response to e₁₂)")
...
PyPlot.title("Populations of E. coli strains (e₁(0)=$e1₀, e₂(0)=$(round(Int,e2₀)), A(0)=$A₀, C(0)=$C₀, b=$b, c=$c, p=$p, μ=$μ)")  #/ magnitudes of immune responses (c=$c 1/h, b=$b 1/h, p=$p 1/(h*cell)
